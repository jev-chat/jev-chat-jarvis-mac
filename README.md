...
验证：离线回归覆盖短句、相邻气泡、多行文本、宽列表下的左右归属、深色合成气泡、选区边界、配置权限及空帧不复用。原生窗口已验证消息区及输入区预览/保存/取消与恢复入口；截图复测覆盖宽窄列表和私聊，单独数字 1/2/3/4 在原始与 1 倍采样下均保留。填入保护通过模拟事件测试验证，未向真实微信输入框写入测试文字。尚未完成真实微信消息到在线模型候选的全流程验收，也未验证所有微信版本及主题。示例截图经打码处理后随 docs/ 提交。
